'''
Quadratic Programming Based Absoluate Policy Optimization
min KL divergence
s.t. advantage term - (mean variance + vaiance mean) >= th
This problem is still solved by CG here.
Reference:
Haotian Xu, Junyu Xuan, Guangquan Zhang, and Jie Lu. 
Quadratic Programming Based Absolute Policy Optimization.
'''
import os
...
from rl_envs.envs.engine import Engine as  rl_envs_Engine
from utils.rl_env_config import configuration
import os.path as osp

from utils.write_csv import write_results_to_csv

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
EPS = 1e-8

mujoco_list =[
    'InvertedDoublePendulum', #(0) states=11,action=1 in (1.0,1.0)
    'InvertedPendulum',       #(1) states=4, action=1 in (-3.0,3.0)
    'Reacher',                #(2) states=11,actions=2 in (-1.0,1.0)
    'Swimmer',                #(3) states=8, actions=2 in (-1.0,1.0)
    'Hopper',                 #(4) states=11,actions=3 in (-1.0,+1.0)
    'HalfCheetah',            #(5) states=17,actions=6 in (-1.0,+1.0)
    'Walker2d',               #(6) states=17,actions=6 in (-1.0,1.0)
    'Pusher',                 #(7) states=23,actions=7 in (-2.0,2.0)
    'Ant',                    #(8) states=27,actions=8 in (-1.0,+1.0)
    'HumanoidStandup',        #(9) states=376,actions=17 in (-0.4,0.4)
    'Humanoid',               #(10)states=376,actions=17 in (-0.4,0.4)
    ]
mujoco_id = 1 ##pick up one of the above environments
...
class QPAPOBuffer:
    ...
    def store(self, obs, act, rew, val, logp, mu=np.nan, logstd=np.nan, logits=np.nan):
        ...
        self.obs_buf[self.ptr] = obs #状态
        self.act_buf[self.ptr] = act #动作
        self.rew_buf[self.ptr] = rew #奖励
...
#======================================================================================
''' QPAPO main body'''
def qpapo(env_fn, omega_1, omega_2, k=10., actor_critic=core.MLPActorCritic, 
        ac_kwargs=dict(), seed=0, 
        steps_per_epoch=4000, epochs=50, gamma=0.99, 
        vf_lr=1e-3, train_v_iters=80, lam=0.97, max_ep_len=1000,
        target_kl=0.01, target_obj=0.05, logger_kwargs=dict(), save_freq=10, backtrack_coeff=1.1,
        backtrack_iters=100, model_save=False, mujoco=None, detailed=True):
    ...
    def mujoco_env_fn(mujoco_name, version='5'):
        ...
        if mujoco_name =='Pusher':
            env_name = mujoco_name + '-v2'
    ...
    buf = QPAPOBuffer(obs_dim, act_dim, local_steps_per_epoch, gamma, lam)
    ...
    def compute_loss_pi_Chebyshev(data, cur_pi):
        ...
        """
        obs, act, disc_adv, adv, logp_old, val = data['obs'], data['act'],\
            data['disc_adv'], data['adv'], data['logp'], data['val']
        ...
        factor = omega_1 * (1 - gamma**2)  #/ k why?
    ...
    def update():
        ...
        print('Primary pi_l_old = ', pi_l_old)
        ...
        min_steplength = np.abs(pi_l_old)*target_obj/(s+EPS)
        print('Min Step Length =', min_steplength)
        
        x_direction = min_steplength * x_hat
        ...
        kl_th = target_kl
        obj_th = pi_l_old - np.abs(pi_l_old)*target_obj
        
        
        # update the policy such that the KL diveragence constraints
        # are satisfied and loss is decreasing
        for j in range(backtrack_iters)[::-1]:
            try:
                stepscale = backtrack_coeff**j
                
                kl, pi_l_new = set_and_eval(stepscale)
            except:
                print('Warning: next update.......')
                break
                #import ipdb; ipdb.set_trace()
            
            if (kl.item() <= kl_th and pi_l_new.item() <= obj_th):
            #if (kl.item() <= kl_th and pi_l_new.item() <= pi_l_old):
                print('KL= ', kl.item(), ' th= ', kl_th)
                print('pi_l_new= ', pi_l_new.item(), ' th= ', obj_th)
                print(colorize(f'Accepting new params at step %d of line search.'%j,\
                               'green', bold=False))
                # update the policy parameter 
                new_param = get_net_param_np_vec(ac.pi) - stepscale * x_direction
                assign_net_param_from_flat(new_param, ac.pi)
                loss_pi, pi_info = compute_loss_pi_Chebyshev(data, ac.pi) 
                # re-evaluate the pi_info for the new policy
                break
            
            if j==0: #backtrack_iters-1:
    ...
    # We will store the running results in a csv file
    epoch_list = []
    epistep_list = []
    allstep_list = []
    alltime_list = []
    ...
    '''one epoch is to collect a set of trajectories'''
    for epoch in range(epochs): #
        
        epoch_ret = 0 # return for each epoch
        epoch_len = 0 # time steps for each epoch
        num_epi = 0
        ...
        for t in range(local_steps_per_epoch):
            ...
            try:
                #
                next_o, r, d_1, d_2, info = env.step(a) #gymnasium返回"terminated"和"truncated"两个标志
                d = d_1 or d_2
            except: 
        ...
        epistep_list.append(epoch_len/num_epi)
        allstep_list.append((epoch+1)*steps_per_epoch)
        alltime_list.append(time.time()-start_time)
        return_list.append(epoch_ret/num_epi)
        print('Episodes:',num_epi, 
              'Average length =', '%0.1f'%float(epoch_len/num_epi),
              'Average return = ', '%0.1f'%float(epoch_ret/num_epi))
        
        #end an epoch
        
    #end all epochs
    return epoch_list, epistep_list, allstep_list, alltime_list, return_list
    
#=================================================================================       
def create_env(args):
    env = rl_envs_Engine(configuration(args.task + "_0Hazards"))
    return env
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, default=mujoco_list[mujoco_id]) #Env
    parser.add_argument('--hid', type=int, default=64) #hidden nodes
    ...
    parser.add_argument('--steps', type=int, default=5000) #time steps/epoch总步数
    parser.add_argument('--max_ep_len', type=int, default=1000) #time steps/episode
    parser.add_argument('--epochs', type=int, default=2000) #epochs       
    parser.add_argument('--exp_name', type=str, default='qpapo') #Agent
    parser.add_argument('--model_save', action='store_true')
    parser.add_argument('--target_kl', type=float, default=0.02) #KL threshold
    parser.add_argument('--backtrack_coeff', type=float, default=1.1) #expanding factor
    parser.add_argument('--backtrack_iters',type=int, default=100) #expanding iterations
    parser.add_argument('--omega1', type=float, default=0.001) #Omega-1      
    parser.add_argument('--omega2', type=float, default=0.005) #Omega-2     
    parser.add_argument('--k', '-k', type=float, default=7.0) #trade-off factor
    parser.add_argument('--detailed', '-d', action='store_false', default=True)        
    parser.add_argument('--mujoco_name', '-m', type=str, default=mujoco_list[mujoco_id])   
    args = parser.parse_args()

    
    if mujoco_id == 0: #Inverted doubel Pendulum(11,1)
        args.omega1,args.omega2,args.backtrack_coeff=5e-5,0.007,1.15 
        seed_list = [0,5,6,7]
        
    if mujoco_id == 1: # #Inverted Pendulum(4,1)
        args.omega1,args.omega2,args.backtrack_coeff=0.0005,0.003,1.1
        seed_list = [0,4,5,7]
        
    if mujoco_id == 2: #Reacher(11,2)
        args.omega1,args.omega2,args.backtrack_coeff=0.0005,0.003,1.1
        seed_list = [0,6,8,9]
        
    if mujoco_id == 3: #Swimmer(8,2)
        args.omega1,args.omega2,args.backtrack_coeff=0.003,0.005,1.1 
        seed_list = [1,3,6,7]
         
    if mujoco_id == 4: #Hopper(11,3)
        args.omega1,args.omega2,args.backtrack_coeff=0.0005,0.01,1.2 
        seed_list = [0,2,5,6]    
        
    if mujoco_id == 5: #Half Cheetah(17,6)
        args.omega1,args.omega2,args.backtrack_coeff=0.0001,0.003,1.2
        seed_list = [1,2,3,4]
        
    if mujoco_id == 6: #Walker2D(17,6)
        args.omega1,args.omega2,args.backtrack_coeff=5e-5,0.01,1.2
        seed_list = [0,2,3,8]
    
    if mujoco_id == 7: #Pusher
        args.omega1,args.omega2,args.backtrack_coeff=5e-5,0.007,1.2 
        seed_list = [1,6,7,8]  
        
    if mujoco_id == 8: #Ant(27,8)
        args.omega1,args.omega2,args.backtrack_coeff=1e-5,0.005,1.2
        seed_list = [1,2,3,7]
        
    if mujoco_id == 9: #Humanoid Standup(376,17)
        args.omega1,args.omega2,args.backtrack_coeff=1e-6,0.005,1.25 
        seed_list = [0,4,7,9]
        
    if mujoco_id ==10: #Humanoid(376,17)
        args.omega1,args.omega2,args.backtrack_coeff=5e-5,0.007,1.2 
        seed_list = [0,2,4,5]
        

    # to execute different seeds
    for seed in seed_list:
        ...
        if args.mujoco_name == None:
            ...
            import gymnasium
            exp_name = args.mujoco_name + '_' + args.exp_name + '_' + 'kl' +\
                str(args.target_kl) + '_' + 'epochs' + str(args.epochs)
        logger_kwargs = setup_logger_kwargs(exp_name, args.seed)

        # whether to save model
        model_save = True if args.model_save else False

        epoch_list, epistep_list, allstep_list, alltime_list, return_list =\
            qpapo(lambda : create_env(args), actor_critic=core.MLPActorCritic,
            ...
            mujoco=args.mujoco_name, detailed=args.detailed)
    
        path_result =  os.path.join('Results', args.task, \
                                 '_'.join([args.task,args.exp_name]))
        if not os.path.exists(path_result): 
          os.makedirs(path_result)
       
    
        eval_fn  = '_'.join(['eval', args.exp_name,args.task,\
                         str(args.hid), str(args.gamma),\
                         str(args.omega1), str(args.omega2),\
                         str(args.backtrack_coeff), str(args.backtrack_iters),\
                         str(args.k),\
                         str(args.epochs), str(args.seed)])
        
        eval_path_fn = ".".join([os.path.join(path_result, eval_fn),'csv'])
        write_results_to_csv(eval_path_fn,epoch_list, epistep_list, allstep_list,\
                        alltime_list, return_list) 
#=======================================================================================

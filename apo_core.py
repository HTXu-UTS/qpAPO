'''
This file is from https://github.com/intelligent-control-lab/Absolute-Policy-Optimization
'''
...
class MLPGaussianActor(Actor):
    ...
    def _d_kl(self, obs, old_mu, old_log_std, device):
        ...
        # debug test to see if P old in the front helps
...
class MLPActorCritic(nn.Module):
    ...
    def __init__(self, observation_space, action_space, 
        ...
        print('hidden layer setting = ',hidden_sizes)

        # policy builder depends on action space
        if isinstance(action_space, Box):
            self.pi = MLPGaussianActor(obs_dim, action_space.shape[0], \
                                       hidden_sizes, activation).to(self.device)
    ...
    def step(self, obs):
        ...
        if isinstance(pi, Normal):
            return a.cpu().numpy(), v.cpu().numpy(), logp_a.cpu().numpy(), \
                pi.mean.cpu().numpy(), torch.log(pi.stddev).cpu().numpy()
